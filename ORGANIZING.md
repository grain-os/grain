...
**`session-logs/`** -- reasoning traces as **Kyri** (`.kyri`); one-clock filenames; newest-first index in `README.md`. Dated logs live under `date/YYYYMMDD/` (molted from `archive/` `20260821.161758`; the room is bounded at 256 flat files). What happened lives here; what is next lives in `TASKS.md`.
...
**`old/`** -- **gone `20260817`, and named here so the absence is legible.** It held this project's predecessor material, the original `urbit/urbit` tree the `.git` history once descended from. That lineage was dropped when the history was re-rooted at Grain's own first commit and every kept commit re-signed; the room went with it. The gratitude is kept rather than carried -- `gratitude/Urbit.md` in the reading library, and [`.claude/rules/urbit-reframe.md`](.claude/rules/urbit-reframe.md), thank the ancestor by name. Reading this entry as a live room is the mistake it now exists to prevent.
...
**Module homes** -- [`rye/README.md`](rye/README.md) - [`rishi/README.md`](rishi/README.md) - [`tally/README.md`](tally/README.md) - [`caravan/README.md`](caravan/README.md) - [`mantra/README.md`](mantra/README.md) - [`comlink/README.md`](comlink/README.md) - [`pond/README.md`](pond/README.md) - [`aurora/README.md`](aurora/README.md) - [`brushstroke/`](brushstroke/) - `linengrow/` (vision until graduation, and a field room until it does), and the rest: each grows a seed toward a running whole.

**`src/`** -- the Glow **userland**: code written in Glow, **three rooms**, each named for the word its own files already wear -- `shape/` the `shape-*.glow` desks (the data-shape museum), `gate/` the `gate-*.glow` desks (the shared arms many desks call), `gen/` the runnable generators. The six elder rooms were renamed on `20260827` ([the charter](active-designing/date/20260827/20260827-174816_the-glow-tree-and-the-rooms-that-say-what-they-hold.md)); `lib/` and `sur/` folded into `gate/` and `shape/`, and `app/`, `sec/` and `til/` keep their READMEs as pointers. The Glow *implementation* (tokenizer, runes, lowerers -- Rye) stays at `glow/`; the desks stay at `glow/gen/` while the O3 breach awaits the maintainer's word (law in [`context/BREACH.md`](context/BREACH.md), state in [`src/gen/README.md`](src/gen/README.md)). Opened as an O1 skeleton `20260727.064038`.
