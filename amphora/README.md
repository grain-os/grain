...
own `vessel.kyri` appears last of all: the crossing lands on a scratch beside it and is renamed
...
| Lap | What |
|-----|------|
| **1 (landed)** | Manifest entry parse (wreck rule) + vessel `.kyri` fields (format - stamp - shoulder - parent - cargo) |
...
**This door is read back, both ways.** `readme_unnamed` walks the roster and asks this page about every row it seats, and it has held at zero since `20260908`. The other direction waited until `20260911`. `readme_named` stood as `guards` minus `readme_unnamed` -- a number describing this page, derived wholly from the roster -- so the roster answered a question about this door on the door's own behalf. So a name this page writes and the roster has yet to seat stayed invisible to the instrument built to keep the two in step, which is how `amphora_device_wire` sat off the roster while this page named it. `readme_claims` counts what this door actually claims, and `readme_unrostered` names every claim still waiting on a roster row, under a ceiling that only falls. Its sibling one room over in this lane, `tools/fixtures/b/kyri_resins_catalog_scan.sh`, has read both directions since it was written -- `uncatalogued` beside `orphan_entries`, `unsealed` beside `orphan_seals`.
