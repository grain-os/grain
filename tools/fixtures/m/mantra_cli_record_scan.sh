...
# %689, door 2 on the maintainer's word `20260916.000449` -- a weave records the
...
for b in "$old"/.mantra/blobs/*; do
  [ -f "$b" ] || continue
  case "$(head -1 "$b")" in
    mantra-weave-20260916.101910) newest="$b" ;;
  esac
done
if [ -z "$newest" ]; then
...
for b in "$bad"/.mantra/blobs/*; do
  [ -f "$b" ] || continue
  case "$(head -1 "$b")" in
    mantra-weave-20260916.101910) sed_inplace '1s/.*/mantra-weave-v9/' "$b" ;;
  esac
done
