...
# parked in four places at once -- a sentence naming the maintainer's word inside living prose, a line
# already recording a word granted, a `YOURS:` row on the operator card, and an `Open doors for
# the maintainer` section -- and found nothing reads them together. "What is waiting on me" was a question
# with four partial answers. This scan gives it one.
#
# THE FOUR PLACES, and how each is told from its neighbor:
#   parked  -- a living line naming the maintainer's word, ruling, or call, carrying a forward-looking verb
#              (waits, awaits, wants, needs, returns, stays, pending, open, parks). This is the
#              open half.
#   granted -- a living line naming "on the maintainer's word" carrying a past-tense granting verb (seated,
...
#   yours   -- a `YOURS:` line, the maintainer's own open question left inside a living pin.
#   doors   -- every bullet under a `## Open doors for the maintainer` heading.
#
...
#
# REPORTED, NEVER GATED. A pending decision is a state, not a fault -- the maintainer's own pace decides
...
while IFS= read -r f; do
  ...
  while IFS= read -r line; do
    ...
    case "$line" in
      *"the maintainer's word"*|*"the maintainer's ruling"*|*"the maintainer's call"*)
        case "$line" in
