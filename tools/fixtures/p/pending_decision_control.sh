...
# throwaway tree.
#
# WHY. A reading that cannot tell "waits for the maintainer's word" from "seated on the maintainer's word" would
...
# a parked decision
This waits for the maintainer's word before it moves; the shape stays open until then.
EOF

cat > "$pen/granted-case.md" <<'EOF'
# a granted decision
**Seated** `20260101.000000` on the maintainer's word -- the row is closed and the number is final.
EOF
...
This page once waited for the maintainer's word, and the word landed the day this was written.
EOF
