...
chapter = (
    ...
    "Bundle and shred stay itinerary modes; Class O paths await the maintainer's word.\n\n"
)
entry = (
...
t = t.replace(
    "And may chapter eight wait for metal, not memory — shred only by the maintainer's word.",
    "And may the rest of chapter eight wait for metal, not memory — shred only by the maintainer's word.",
    1,
)
p.write_text(t)
