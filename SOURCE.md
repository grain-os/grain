...
**By:** Riyo, in the radiant voice, with **the maintainer** as coauthor (Identity Remake `20260729.141658`; standing writing voice restored to Riyo `20260729.205200`)
...
```bash
git clone https://github.com/acme-owner/grain.git ~/grain
cd ~/grain
```

Public HTTPS, no fork, no keys yet. This pier’s living public clone is **`acme-owner/grain`**. A second GitHub remote, `acme-owner/grain`, may exist as the agentic research lane (`origin` on this pier) — see [`context/REMOTE_ROSTER.md`](context/REMOTE_ROSTER.md). Living Cursor sends **dual-push** both remotes when both exist. **Codeberg is retired** from living push (Terms July 2026) until a new second forge is chosen.
...
**This pier** uses GitHub `acme-owner/grain` as the public clone you just fetched, and may also carry `acme-owner/grain` as a second remote for agentic research. Canonical remote count: [`context/REMOTE_ROSTER.md`](context/REMOTE_ROSTER.md).
