...
**Status:** Proposed -- three decisions, none taken. Each changes what the product admits, so each waits for the maintainer's word.
...
the maintainer asked whether the closed `counsel/` room should be revived for pages of this kind, or whether
...
| Reading | Files |
|---|---:|
| Pages parking a decision on the maintainer's word | **80** |
| Pages recording a word already granted | 44 |
| `YOURS:` lines on the operator card | 6 |
| `Open doors for the maintainer` sections | 1 |
...
says a decision returns to, waits for, or stays the maintainer's word, printing each with its page and its
...
and an `Open doors for the maintainer` bullet beside them -- all four places, one reading, reported and
...
decisions themselves are proposals awaiting the maintainer's word rather than projections, so they carry no
