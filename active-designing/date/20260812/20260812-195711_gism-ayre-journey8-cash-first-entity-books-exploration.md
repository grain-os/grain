...
3. **J8r3 — the closed period travels.** An entity's period folds to a sealed statement that renders to a `format entity-period-v1` Kyri record and parses back byte-for-byte, still balancing — a closed period is testimony a recipient checks offline (mirroring the receipt and certificate travel rungs).
