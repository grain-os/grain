...
5. Hold for the maintainer’s **named supersede build word** — then freeze a release candidate · record `soak_start_nib` · soak under N · write `kyri-resins/pond-supersede-exit.kyri` with content · **master-sign** on the host (cold key only):

   ```
   gpg --local-user [redacted-fingerprint]! --armor --detach-sign kyri-resins/pond-supersede-exit.kyri
   ```
