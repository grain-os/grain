...
The dated notes in `active-designing/`, the counsel shelf, and the Kyri session logs **are** the raw layer: immutable by law and machine-enforced. What is missing is the compression layer. Today alone scattered the proven-seat and enclosure stories across roughly eighteen notes — the exact case where a page pays.
...
1. **Kyri -> Kyri unification.** Move every living Kyri reference to Kyri (voice, notation, compressed receipts, preferred variant under one name). New logs are already `.kyri`; the 2,163 dated `.kyri` files stay under the one-clock law and tools read both. The deep rename of dated bytes is a separate circled step.
