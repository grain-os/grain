...
- **r2 — the knot travels.** A `format cord-knot-v1` Kyri record (issued, taxed, one `star` line per reserved name, the digest as hex) renders and parses back **byte-for-byte**; the recovered Knot verifies, and a suffix continued onto it folds the identical supply; a tampered field refuses `KnotCorrupt` on rebuild, an unknown field refuses (never dropped). Composes the `cord_kyri` reader/writer idiom.
