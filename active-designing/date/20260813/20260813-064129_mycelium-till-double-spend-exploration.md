...
- **r2 — the verdict travels.** Render a resolved `Till` to a `format till-v1` Kyri record — each winning draw a `drew <signer-pk-hex> <amount> <pos>` line, each spent-out loss a `short <signer-pk-hex> <amount> <pos>` line — and parse it back byte-for-byte, the recovered Till answering the identical counts and totals, so a dev-net's treasury verdict crosses as readable text.
