...
- **r2 — the verdict travels.** Render a resolved `Purse` to a `format purse-v1` Kyri record — each account a `holds <pk-hex> <balance>` line, each winning move a `moved <from-hex> <to-hex> <amount> <pos>` line, each spent-out loss a `short <from-hex> <amount> <pos>` line — and parse it back byte-for-byte, the recovered Purse answering the identical balances, counts, and totals, so a dev-net's per-account verdict crosses as readable text.
