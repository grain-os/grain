...
**Voice:** Kyri
**Status:** GRANTED `20260827` (`approve all doors`) -- the `src/` rooms, the `loops/` room, the `glow/gen/` letter fold, and FORA31 have all landed; the **deep debride** waits on its own precondition and on the maintainer's word (custody gate %5). What landed differs from what was proposed in two places, both recorded below.
