# 966 - Brix vs Kyri -- When to Compose and When to Notate

*Two formats, two purposes, one value model. Brix declares what a system is made of -- it is the composition language, the way Nix declares a package set for SixOS and s6. Kyri carries values as text -- it is the data notation, the way JSON carries data between programs. This research draws the boundary precisely, grounded in how Nix and infuse separate composition from data, and how ZON's token efficiency shapes when Kyri earns its place.*
...
| **File extension** | `.brix` | `.kyri` |
| **Evaluated?** | Yes -- Brix expressions are evaluated to produce build plans, service trees, composition results | No -- Kyri is parsed, not evaluated. A `.kyri` file is data, not a program. |
| **Has functions?** | Yes (growing toward parameterized declarations, like Nix infuse's parameterized overlays) | No. Kyri is key-value. No functions, no expressions, no control flow. |
| **Has dependencies?** | Yes -- a Brix declaration names what it depends on. Brix interfaces with Mantra for versioned inputs and targets Aurora/Tally for boot stages and memory budgets. | No. A Kyri file is self-contained. |
| **Backed by a store?** | Yes -- Brix has its own content-addressed build artifact cache (like Nix's `/nix/store`), currently inside `.mantra/blobs/` | No. Kyri is a text format; it has no store. |
...
The lesson: **the composition language evaluates to data.** Brix evaluates to Kyri. A `.brix` file is a program that, when run, produces a `.kyri`-shaped value -- a record of keys and values that names what was composed.
...
The lesson: **the composition language has a richer syntax than the data it produces.** Brix may have overlays, parameterization, and imports; the result is always a Kyri-shaped record.
...
The lesson: **Brix declares what Caravan supervises.** The `.brix` declaration produces a service tree; Caravan reads the tree and starts the services. The service tree itself is a Kyri value -- keys and values naming what to run, what capabilities to grant, what gardens to bound.
...
Use Kyri (`.kyri`) when you are **carrying a value as text** -- settings, metadata, headers, configuration that is read but not evaluated:
...
- **Mantra commit metadata** -- `parent <hash>`, `weave <hash>` (already Kyri-shaped)

A `.kyri` file is parsed, not evaluated. It has no functions, no imports, no expressions. It is data.
...
```
Brix (composition) --evaluates---> Kyri (data)
                                    |
...
Brix produces Kyri the way Nix infuse produces attribute sets. The composition language is richer; the data notation is simpler. Every module that consumes configuration reads Kyri -- the universal data format of the family.

---

## Token Efficiency -- When Kyri Saves Money

ZON demonstrated that format efficiency directly affects LLM costs: 35-50% fewer tokens than JSON for the same data. Kyri achieves similar savings through the same design pressure -- minimal syntax, no quotes, no braces.

**When token efficiency matters (use Kyri):**
...
The rule: **if a value is read many times or sent across a wire, Kyri. If a value is evaluated once to produce other values, Brix.**

---

## Why Kyri Aligns with Zig and Tiger Style, Not with Clojure
...
Yet Kyri aligns with Zig and Tiger Style rather than with Clojure. Where EDN serves a Lisp -- parenthesized, homoiconic, reader-macro-extensible -- Kyri serves a systems language whose values are explicitly sized, whose assertions guard every boundary, and whose discipline is TAME: safety first, performance second, joy third.

| Design choice | EDN (Clojure) | Kyri (Rye) |
|--------------|---------------|------------|
...
| Philosophy | Homoiconicity -- code is data | Separation -- Brix is the code; Kyri is the data |
| Alignment | Functional, Lisp tradition | Systems, Tiger Style / TAME discipline |

Kyri is simpler than EDN on purpose. A systems notation does not need reader macros or nested maps in its seed form. It needs one field per line, stated types by convention, and a parser smaller than the format specification. The simplicity is earned -- the same way Tiger Style says simplicity is the hardest revision.
...
*May Brix declare what the system is made of, and may Kyri carry the values that result -- each on its own ground, each as plain as its purpose demands. The composition language evaluates; the data notation persists. Both speak the one value model, and neither needs more syntax than the meaning requires.*
