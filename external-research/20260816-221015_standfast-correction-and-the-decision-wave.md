...
- **The session-log rule sync.** Matched to the profile of record: Claude Code, `claude-opus-4-8`, the a cloud provider VPS pier with the a tablet as the hand device, and the `.kyri` filename default (the notation named Kyri). This session's own four mis-attributed logs were corrected to the truth -- a factual model-field fix the rule allows.
