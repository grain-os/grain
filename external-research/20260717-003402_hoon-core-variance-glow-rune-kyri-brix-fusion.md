# Hoon's Core Variance -- Gold, Iron, Lead, Zinc -- and What Glow's Rune Vision Should Do With It, Fused with Kyri and Brix
...
## Part 6 -- The Fusion with Kyri: Where Variance Does and Does Not Apply

Kyri itself -- flat key-value text, no braces, no nesting, no code -- carries no battery at all; a Kyri file is pure payload with no arms attached to it. **Variance, as Hoon defines it, is a property of cores, and Kyri values are never cores** -- so the honest answer is that Kyri itself needs nothing from this whole document. The place the question actually lands is one seam further along: **when Brix reads a Kyri value and binds it into a system's own configuration, that binding is a mold check** -- is this text, read as this shape, compatible with what the consuming module declared it needs? This is Hoon's own `nest`/`mull` machinery's actual job (proving one type's own shape compatible with another's, quoted in Part 2), simply applied to a Kyri-carried value being checked against a Brix-declared mold rather than to one core's payload checked against another's. **The fusion, stated plainly: Kyri is the untyped wire; Brix's own mold-checking of a Kyri value at the moment it enters a bond is where a Hoon-style nest check belongs, if Glow ever wants one -- never inside Kyri's own notation, which stays deliberately, permanently untyped.**
...
- It does not change Kyri's own notation in any way -- Part 6's conclusion is that Kyri needs nothing from this thread at all, and names exactly where the real question actually belongs instead.
...
- `../active-designing/yonder/20260621-063912_kyri-notation.md` -- Kyri's own design brief, the ground for Part 6.
