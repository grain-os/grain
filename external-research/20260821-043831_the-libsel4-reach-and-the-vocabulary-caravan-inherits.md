...
2. **Open the generator gate** when convenient, by adding `jinja2` and `ply` to the pier's Python environment, which unlocks `types.h`, `syscalls.h`, `bootinfo.h`, `faults.h`, and the `sel4.h` umbrella in one step. Environment change, so it is the maintainer's word.
