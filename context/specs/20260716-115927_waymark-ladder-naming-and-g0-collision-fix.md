...
The prior session already spotted this and named it honestly without yet fixing it: *"That machinery belongs to a wholly unrelated Genode/seL4 proven-seat guest ladder that happens to reuse the same G0-G3 numbering as the GrapheneOS/Pixel ladder -- a real naming collision, kept straight rather than conflated"* (`session-logs/20260716-110152_framework-ubuntu-host-arrives-slc-green.kyri`). This document closes that open thread.
...
5. Assert the drawn word is not already a seated four-letter name in `context/LEXICON.md` (`BRIX`, `KYRI`, `POND`, `MUR`, `SEVA`, `MONA`, `POOL`, `QUIN`, `RIYO`, `REYA`, `TREY`, `TERA`, `TRIZ`, `OJJO`, `GLOW`, `RHYZ`, `PEAL` -- plus retired `MALA`; the only entries of that same length). A collision here is vanishingly unlikely at 1-in-5,526 odds, and the script fails loudly rather than silently if it ever happens; the fix is to salt the input (append `-2`) and redraw.
...
- `session-logs/20260716-110152_framework-ubuntu-host-arrives-slc-green.kyri` -- where the collision was first spotted and named, not yet fixed.
