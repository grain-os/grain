...
**Status:** Living -- seats `GLOW_PROFILE.kyri` and the generic-Acme adaptation of the living entry docs
...
That one place is a new root config file, **`GLOW_PROFILE.kyri`** -- the sibling to `GLOW_HOST.kyri`.

## The Two Root Profiles, Side by Side

The fork now carries two root Kyri configs, each declaring one axis of "this particular clone," each with a committed `.template` and a gitignored filled copy:

| File | Declares | Read by |
|---|---|---|
| **`GLOW_HOST.kyri`** | the **host**: os, arch, toolchain paths | `tools/glow_host_run.sh` (refuses a mismatched toolchain) |
| **`GLOW_PROFILE.kyri`** | the **contributor**: professional name, forge handles, clock, session-log defaults, placeholder identities | the session-logs convention and the README's setup path cite it as the single record of "who / when / how this clone is configured" |

Both are the Kyri-level sibling to `.claude/rules` and `.cursor/rules`: those configure *agent behavior*; these configure the *host* and the *contributor*. All four together are how a fresh clone declares itself without editing a single shared, teachable document.
...
- **`context/README.md`** -- the stale `~/acme-owner` workspace path corrected to the Glow OS repository; a line added naming `GLOW_PROFILE.kyri` as where the specific contributor lives; the reader addressed generally; `the maintainer` -> `the maintainer` on this touch.
...
- **`ORGANIZING.md`** -- a `GLOW_PROFILE.kyri` entry added beside the existing `GLOW_HOST.kyri` entry.
...
- Whether a small tool should *read* `GLOW_PROFILE.kyri` (the way `glow_host_run.sh` reads `GLOW_HOST.kyri`) -- for example, to stamp session-log headers from the profile automatically. Today the profile is a declarative record that documents cite; wiring a reader is a natural, separately-witnessed next step if the need proves real, not built ahead of it.
