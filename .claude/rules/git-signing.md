...
GitHub's commit-message renderer turns any `#<number>` into a link to the issue or PR of that number, exactly as it does for `@name` above. This is a documented fact rather than a hypothetical: `acme-owner/grain` carries pull requests numbered into the eighties, so `REDS %80` in a commit body has been rendering as a link to an unrelated equinox PR. A reader following it lands somewhere other than what the sentence meant.
