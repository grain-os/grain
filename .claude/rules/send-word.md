...
3. **Merge to `main`**: fast-forward when clean (`git checkout main` · `git merge --ff-only <branch>` · `git push origin main`), preserving every signed commit. Never force-push or amend unless the maintainer says so explicitly.
